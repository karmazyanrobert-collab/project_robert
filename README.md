...
## 15. Если GitHub всё ещё показывает конфликтующие файлы

В этой версии все ранее отмеченные файлы сохранены без Git merge-маркеров и дополнительно проверяются тестом `test_conflicting_files_have_no_merge_markers`:
...
Если интерфейс GitHub всё ещё показывает конфликт, это означает не наличие стандартных merge-маркеров в файлах, а расхождение ветки PR с целевой веткой. В таком случае нужно обновить PR-ветку относительно целевой ветки и оставить текущую русскоязычную demo-версию этих файлов.
